...
public class SidePanel extends JPanel {
	...
	public void paintComponent(Graphics g) {
		 ...
		 */
		g.setFont(new Font("맑은 고딕",Font.ITALIC, 24));
		g.drawString("정보:", SMALL_INSET, offset = STATS_INSET);
		g.setFont(new Font("맑은 고딕",Font.ITALIC, 12));
		g.drawString("레벨: " + tetris.getLevel(), LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("스코어: " + tetris.getScore(), LARGE_INSET, offset += TEXT_STRIDE);

		/*
		 * Draw the "Controls" category.
		 */
		g.setFont(new Font("맑은 고딕",Font.ITALIC, 24));
		g.drawString("조작법", SMALL_INSET, offset = CONTROLS_INSET);
		g.setFont(new Font("맑은 고딕",Font.ITALIC, 12));
		g.drawString("A - 왼쪽으로 이동", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("D - 오른쪽으로 이동", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("Q - 반시계 방향 회전", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("E - 시계방향 회전", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("S - 블럭 떨어뜨리기", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("P - 일시 정지", LARGE_INSET, offset += TEXT_STRIDE);

		/*
		 * Draw the next piece preview box.
		 */
		g.setFont(new Font("맑은 고딕",Font.ITALIC, 16));
		g.drawString("다음 조각:", SMALL_INSET, 70);
